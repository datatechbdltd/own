    ...
    <div class="contentbar">
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
                ...
                <div class="card-body">
                    <form action="{{ route('sales.payment.update', $payment) }}" method="post" class="row" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="form-group col-12">
                            ...
                            <select class="select2-single form-control select2-hidden-accessible" name="offline_payment_method" id="offline_payment_method" data-select2-id="1" tabindex="-1" aria-hidden="true">
                                ...
                                @foreach($offline_payment_methods  as $offline_payment_method)
                                    <option @if($payment->payment_offline_method_id == $offline_payment_method->id) selected @endif value="{{ $offline_payment_method->id }}">{{ $offline_payment_method->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-12">
                            ...
                            <select class="select2-single form-control select2-hidden-accessible" name="invoice" id="invoice" data-select2-id="1" tabindex="-1" aria-hidden="true">
                                ...
                                @foreach($invoices  as $invoice)
                                    <option @if($payment->invoice_id == $invoice->id) selected @endif value="{{ $invoice->id }}">{{ $invoice->invoice_id }}</option>
                                @endforeach
                            </select>
                            @error('offline_payment_method')
                            <div class="alert alert-danger" role="alert">
                        ...
                        <div class="form-group col-12">
                            ...
                            <input type="number" class="form-control" id="amount" name="amount" placeholder="Amount" value="{{ $payment->amount }}">
                            @error('amount')
                            <div class="alert alert-danger" role="alert">
                                {{ $message }}
                            </div>
