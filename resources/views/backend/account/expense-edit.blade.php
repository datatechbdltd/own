    ...
    <div class="contentbar">
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
                ...
                <div class="card-body">
                    <form action="{{ route('account.expense.update', $expense) }}" method="post" class="row" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="form-group col-12">
                            ...
                            <select class="select2-single form-control select2-hidden-accessible" name="expense_category" id="expense_category" data-select2-id="1" tabindex="-1" aria-hidden="true">
                                ...
                                @foreach($expense_categorys as $expense_category)
                                    <option @if($expense->category_id == $expense_category->id) selected @endif value="{{ $expense_category->id }}">{{ $expense_category->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-12">
                            ...
                            <textarea class="summernote-description" id="description" name="description">{!! $expense->description !!}</textarea>
                            @error('description')
                            <div class="alert alert-danger" role="alert">
                                {{ $message }}
                            </div>
                        ...
                        <div class="col-lg-12">
                            <div class="card m-b-30">
                                <div class="card-body">
                                    <img src="{{ asset($expense->image ?? get_static_option('no_image')) }}" class="img-fluid" alt="Responsive image">
                                </div>
                            </div>
                        </div>
                        <div class="form-group col-xl-6 col-md-6">
