    ...
    <div class="breadcrumbbar">
        <div class="row align-items-center">
            <div class="col-md-8 col-lg-8">
                <h4 class="page-title">{{ __('Create lead service') }}</h4>
                <div class="breadcrumb-list">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ config('app.name') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ __('Lead service create') }}</li>
                    </ol>
                </div>
            </div>
                        ...
                        <div class="table-responsive">
                            <table id="default-datatable" class="display table table-striped table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $service->leads->count() ?? '0' }}</td>
                                    <td>
                                        <input type="hidden" class="id" value="{{ $service->id }}">
                                        <button type="submit" class="btn btn-warning edit-btn">Edit</button>
                                        <button type="submit" class="btn btn-danger" onclick="delete_function(this)" value="{{ route('lead.leadService.destroy', $service) }}">Delete</button>
                                    </td>
                                </tr>
                                @endforeach
                                </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="varying-modal-label">Lead Service</h5>
...
@push('script')
    <script>
        ...
        $('.submit-btn').click(function() {
            var url, type = "";
            if ($(this).val() == 'add'){
                url = "{{ route('lead.leadService.store') }}";
                type= "POST"
            }else{
                url =  "/lead/leadService/"+$('#hidden-id').val();
                type = "PATCH";
            }
            $.ajax({
                url: url,
