    ...
    <div class="breadcrumbbar">
        <div class="row align-items-center">
            <div class="col-md-8 col-lg-8">
                <h4 class="page-title">{{ __('Create lead category') }}</h4>
                <div class="breadcrumb-list">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ config('app.name') }}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ __('Lead category create') }}</li>
                    </ol>
                </div>
            </div>
                        ...
                        <div class="table-responsive">
                            <table id="default-datatable" class="display table table-striped table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $category->leads->count() ?? '0' }}</td>
                                    <td>
                                        <input type="hidden" class="id" value="{{ $category->id }}">
                                        <button type="submit" class="btn btn-warning edit-btn">Edit</button>
                                        <button type="submit" class="btn btn-danger" onclick="delete_function(this)" value="{{ route('lead.leadCategory.destroy', $category) }}">Delete</button>
                                    </td>
                                </tr>
                                @endforeach
                                </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="varying-modal-label">Lead Category</h5>
...
@push('script')
    <script>
        ...
        $('.submit-btn').click(function() {
            var url, type = "";
            if ($(this).val() == 'add'){
                url = "{{ route('lead.leadCategory.store') }}";
                type= "POST"
            }else{
                url =  "/lead/leadCategory/"+$('#hidden-id').val();
                type = "PATCH";
            }
            $.ajax({
                url: url,
